...
typedef struct TDB_PROXY_SETTING
{
...
//登录回报
typedef struct TDBDefine_ResLogin
{
    char szInfo[64];             //信息
    int  nMarkets;               //支持市场个数
...
///行情消息结构体////////////////////////////////////////////////////
//K线数据 KLine
typedef struct TDBDefine_ReqKLine
{
...
typedef struct TDBDefine_KLine
{
    char chWindCode[32];            //万得代码(AG1312.SHF)
    char chCode[32];                //交易所代码(ag1312)
...
typedef struct TDBDefine_ReqTick
{
    char chCode[32];    //证券万得代码(AG1312.SHF)
	char chMarketKey[24];		//市场设置,如：SH-1-0;SZ-2-0
...
//逐笔成交数据 Transaction
typedef struct TDBDefine_ReqTransaction
{
    char chCode[32];            //证券万得代码(AG1312.SHF)
	char chMarketKey[24];		//市场设置,如：SH-1-0;SZ-2-0
...
typedef struct TDBDefine_Transaction
{
    char    chWindCode[32];     //万得代码(AG1312.SHF)
    char    chCode[32];         //交易所代码(ag1312)
...
typedef struct TDBDefine_Order
{
    char chWindCode[32];        //万得代码(AG1312.SHF)
    char chCode[32];            //交易所代码(ag1312)
...
typedef struct TDBDefine_OrderQueue
{
    char    chWindCode[32];         //万得代码(AG1312.SHF)
    char    chCode[32];             //交易所代码(ag1312)
    int     nDate;                  //日期（自然日）格式YYMMDD
...
typedef struct TDBDefine_AddFormulaRes
{
    int nErrLine;           //错误行
...
typedef struct TDBDefine_FormulaItem
{
    char chFormulaName[28];    //指标公式名称
    char chParam[132];         //参数
}TDBDefine_FormulaItem;

typedef struct TDBDefine_DelFormulaRes
{
    char chFormularName[28];    //删除的指标名称
    char chInfo[68];            //删除信息
}TDBDefine_DelFormulaRes;

typedef struct TDBDefine_CalcFormulaRes
{
    char chWindCode[32];        //万得代码(AG1312.SHF)
    char chCode[32];            //交易所代码(ag1312)
...
//请求计算指标公式
typedef struct TDBDefine_ReqCalcFormula
{
